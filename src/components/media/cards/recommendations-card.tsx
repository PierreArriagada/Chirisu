      ...
      <CardContent className="space-y-4">
        {items.map(item => (
            <Link key={item.id} href={`/${item.type.toLowerCase().replace(/ /g, '-')}/${item.slug}`} className="block group">
                <Card className="flex items-center gap-4 p-3 overflow-hidden bg-card text-card-foreground transition-all duration-200 hover:bg-accent/50 hover:shadow-md">
