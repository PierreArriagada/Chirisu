...
'use client';
import { TopMediaList, TopMediaSection, GenreGridCard } from "@/components/catalog";
import { TopRankingSlideshow } from "@/components/rankings";
...
const popularGenres: Record<MediaType, string[]> = {
    ...
    'Donghua': ["Acción", "Aventura", "Fantasía", "Histórico", "Artes Marciales", "Sobrenatural"],
    'Manhua': ["Acción", "Artes Marciales", "Fantasía", "Histórico", "Aventura", "Romance"],
    'Manhwa': ["Acción", "Fantasía", "Drama", "Aventura", "Psicológico", "Deportes"],
...
export default function AnimePageClient({ mediaType = "Anime" }: { mediaType?: MediaType }) {
    ...
    const showVideoSections = mediaType === 'Anime' || mediaType === 'Donghua';
        ...
        try {
            ...
            if (dataLatest.success && dataLatest.data) {
                ...
                const latest: TitleInfo[] = dataLatest.data.map((item: any) => ({
                    ...
                    ranking: undefined, // ✅ No mostrar ranking en "Latest" (son recientes, no rankeados)
                    commentsCount: 0,
                    listsCount: 0,
                }));
            ...
            // Próximos estrenos - obtener solo los que tienen status "not_yet_aired"
            if (showVideoSections) {
                try {
                    const responseUpcoming = await fetch(`/api/upcoming?type=${apiType}&limit=8`);
                    const dataUpcoming = await responseUpcoming.json();
                    console.log('📊 Respuesta Upcoming:', dataUpcoming);
                    
                    if (dataUpcoming.upcoming && dataUpcoming.upcoming.length > 0) {
                        console.log(`✅ Próximos estrenos recibidos: ${dataUpcoming.upcoming.length} items`);
                        const upcomingItems: TitleInfo[] = dataUpcoming.upcoming.map((item: any) => ({
                            id: item.id.toString(),
                            slug: item.slug || item.id.toString(),
                            title: item.title,
                            type: mediaType,
                            ...
                            imageHint: item.title,
                            rating: item.averageScore || 0,
                            ranking: undefined, // No mostrar ranking para próximos estrenos
                            commentsCount: 0,
                            listsCount: 0,
                        }));
                        setUpcomingReleases(upcomingItems);
                    } else {
                        console.warn('⚠️ No hay próximos estrenos disponibles');
                        setUpcomingReleases([]);
                    }
                } catch (error) {
