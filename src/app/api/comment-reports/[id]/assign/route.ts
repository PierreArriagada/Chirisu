...
export async function POST(
  ...
  try {
    ...
    const checkResult = await pool.query(
      'SELECT id, assigned_to FROM app.comment_reports WHERE id = $1',
      [id]
    );
    ...
    await pool.query(
      `INSERT INTO app.audit_log (user_id, action, resource_type, resource_id, new_values)
       VALUES ($1, 'assign_comment_report', 'comment_report', $2, $3)`,
      [payload.userId, id, JSON.stringify({ assigned_to: payload.userId, status: 'reviewing' })]
    );

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Error en POST /api/comment-reports/[id]/assign:', error);
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO app.audit_log (user_id, action, resource_type, resource_id, old_values)
       VALUES ($1, 'release_comment_report', 'comment_report', $2, $3)`,
      [payload.userId, id, JSON.stringify({ assigned_to: report.assigned_to })]
    );

    return NextResponse.json({ success: true });
  } catch (error: any) {
    console.error('Error en DELETE /api/comment-reports/[id]/assign:', error);
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
}
