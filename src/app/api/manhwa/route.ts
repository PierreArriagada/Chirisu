/**
 * ========================================
 * API ROUTE: MANHWA (Korean Comics)
 * ========================================
 * 
 * GET /api/manhwa?limit=20&offset=0&sort=ranking
 * 
 * Obtiene listado de manhwa con filtros y ordenamiento
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    const manhwa = result.rows;
    const total = parseInt(countResult.rows[0]?.total || '0');

    console.log(`✅ GET /api/manhwa: ${manhwa.length} items (total: ${total})`);
    ...
    console.error('❌ Error en GET /api/manhwa:', error);
    return NextResponse.json(
      { error: 'Error al obtener manhwa' },
      { status: 500 }
    );
  }
}
